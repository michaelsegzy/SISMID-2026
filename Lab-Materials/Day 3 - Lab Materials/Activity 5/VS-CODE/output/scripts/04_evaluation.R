...
# reference_date x horizon, using the scoringutils package. Reports WIS, MAE,
...
# alongside WIS and the median absolute error (MAE).
...
# FluSight-style metric labels, sort, and round WIS/MAE to one decimal.
...
# to show how skill changes with lead time. The mean of the 0/1 coverage_95 is
# the empirical 95% coverage rate.
summary_out <- scores_out %>%
  ...
  summarise(
    ...
    MAE_mean         = round(mean(MAE), 1),
    MAE_min          = round(min(MAE), 1),
    MAE_max          = round(max(MAE), 1),
