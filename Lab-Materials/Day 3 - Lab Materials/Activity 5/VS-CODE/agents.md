...
**WIS**, **MAE**, and **95% PI coverage**. Do the scoring with the
...
`model`), and `score()` it. Take `wis` (WIS), `ae_median` (MAE), and the 95%
interval-coverage metric. Output table columns, in order: `reference_date`,
`horizon`, `target_end_date`, `observed`, `WIS`, `MAE`, `coverage_95`.
