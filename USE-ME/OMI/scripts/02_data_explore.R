...
p <- ggplot(plot_df, aes(x = week, y = value, fill = season_label)) +
  annotate(
    "rect",
    xmin = season_xmin,
    xmax = season_xmax,
  ...
  scale_x_date(date_breaks = "6 weeks", date_labels = "%Y-%m-%d") +
  theme_minimal() +
  theme(
